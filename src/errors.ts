...
export class UnknownGlobalTokenError extends DiError {
	constructor(token: Token) {
		super(
			`Unknown token in global module: ${getTokenName(token)}. Make sure that singleton dependencies also are globally defined`,
		)
	}
}

export class DecoratorUsageError extends DiError {
	constructor(name: string, kind: string) {
		super(`@${name} can only be applied to a ${kind}`)
	}
}
